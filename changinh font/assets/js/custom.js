async function saveLead(e,t,l,n,i,o){
   if (e.trim() === "") {
    alert("Name Field Cannot be empty");
    return false;
}
    if("+91"===l){let a=n.replace(/\D/g,"");if(10!==a.length||/^[0-5]/.test(a))return alert("Invalid phone number. For Indian numbers, enter a valid 10-digit number starting with 6-9."),!1}
if ("+91" !== l){
        let a = n.replace(/\D/g, "");
        if (5 >= a.length)
            return (
                alert("Invalid phone number. Mobile number must contain only numbers (no letters or symbols) and be long enough to place a call."), !1
            );
    }
let u=document.querySelector(".dropnOtp");if(u&&""===u.value&&parseInt(n))try{let r=await fetch("https://api.homesfy.in/api/users/send_otp",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({phone:parseInt(n)})});if(r.ok){let d=await r.json();u.value=d.otp}}catch(m){console.error("Error sending OTP:",m)}let s=[".FillOtp",".FillOtp1",".FillOtp2"].map(e=>document.querySelector(e)).filter(e=>e);if(!u||""!==u.value&&s.some(e=>u.value===e.value)){const leadNationality = (l === '+91') ? 1 : 2;let p=queryForm(),c=await getIpAddress(),g=deviceData(),f=browserData(),y={name:e,email:t,country_code:l,number:n,tracking_lead_id:i,nationality:leadNationality,source_id:31,project_id:projectId,Digital:{user_device:g,user_browser:f,campaing_type:p?p.utmcampaign:null,launch_name:"",client_ipaddress:c,client_pref:o}, more_fields: {is_loan: is_loan}};p&&(y.Utm={utm_medium:p.utmmedium,utm_source:p.utmsource,utm_content:p.utmcontent,utm_term:p.utmterm}),1===is_magnet&&(y.is_magnet=is_magnet,y.magnet_id=magnet_id,y.source_id=49),
localStorage.setItem("submittedCountryCode", l);
